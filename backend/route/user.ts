...
userRouter.get("/getmessage/:otherEmail", auth, async (req: meget, res: Response): Promise<any> => {
  try {
    const myEmail: string = req.email!;
           if (!myEmail) {
            return res.status(401).json({
                success: false,
                msg: "Unauthorized fetch"
  ...
  data: { isread: true }
    ...
    const formattedMessages: MessagePack[] = rawMessages.map((msg: any) => ({
      id: msg.id,
      sender: msg.sender,
...
userRouter.get("/prepost", auth, async (req: meget, res: Response): Promise<any> => {
    try {
        ...
        const prejobs: prepost[] = jobs.map((job: any) => ({
            id: job.id,
...
userRouter.post("/lc-refresh", auth, async (req: meget, res: Response): Promise<any> => {
    ...
    `;
        ...
        const resRecent = await fetch("https://leetcode.com/graphql", {
            method: "POST",
            headers: { "Content-type": "application/json" },
            body: JSON.stringify({
                query: recentProblemQuery,
                variables: { username },
            }),
        });
      ...
      `;
            ...
            const resDetails = await fetch("https://leetcode.com/graphql", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            const detailJson = await resDetails.json();
            ...
            const newProblem = {
                ...
                xp: xp,
                tags: q.topicTags.map((t: any) => t.name).join(", "),
                timestamp: new Date(Number(problem.timestamp) * 1000),
            };

            await prisma.solvedProblemDetails.upsert({
                where: {
                    userid_titleslug: {
                        userid: user.id,
        ...
        const resStats = await fetch("https://leetcode.com/graphql", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                query: `
        ...
        return res.json({ success: true, details: result, savedProblems: submissions.length });
...
interface addSolvedProblem {
    title: string,
    difficulty: string
}

userRouter.get("/getSolvedProblems", auth, async (req: meget, res: Response): Promise<any> => {
    try {
        const email: string | null = req.email!;
        ...
        const problems = await prisma.solvedProblemDetails.findMany({
            where: { userid: user.id },
        ...
        console.error("Fetch solved problems error:", e);
        return res.status(500).json({
            success: false,
            msg: "Server error"
        });
...
userRouter.get("/ai/weekly-plan", auth, async (req: meget, res: Response): Promise<any> => {
    try {
        ...
        const solved = await prisma.solvedProblemDetails.findMany({
            where: { userid: user.id },
            select: {
                difficulty: true,
                tags: true,
        ...
        let Medium: number = 0
        let Hard: number = 0
        solved.map((x: { difficulty: string }) => {
            if (x.difficulty === "Easy") Easy += 1
            else if (x.difficulty === "Medium") Medium += 1
            else Hard += 1
        })


        const topicCount: Record<string, number> = {};
        solved.forEach(({ tags }: { tags: string }) => {
            const topics = tags.split(',').map((tag: string) => tag.trim());
        ...
        for (const topic in topicCount) {
            topicLines += `- ${topic}: ${topicCount[topic]}\n`;
        }
        const prompt = `User Solved:
                         -${Easy} Easy,
                         -${Medium} Medium,
                         -${Hard} Hard
                         ...
                         Generate a 7-day DSA study plan based on this user's current practice. Mention which topics to revise more. Suggest number of questions and difficulty each day
                         Focus more on weak areas. Include:
                        - Topics per day
                        - Number of problems
                        - Difficulty split
                        Keep it motivating, structured and realistic.
                         `;
        ...
        return res.status(200).json({
            success: true,
            plan: result.candidates?.[0]?.content?.parts?.[0]?.text || "No plan generated",
        });
...
userRouter.get("/ai/motivation", auth, async (req: meget, res: Response): Promise<any> => {
    try {
        ...
        const prompt = `User Solved:
                        ...
                        Write a 2-line motivational message that encourages them to keep going and stay consistent.
                        Make it personal, positive, and short.
                         `;
