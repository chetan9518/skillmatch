...
export function Search() {
  ...
  useEffect(() => {
    async function fetch() {
      try {
        const token = localStorage.getItem("token");
        if (!token) return;
        const result = await axios.get(`${API}/user/fetchuser`, {
          headers: {
  ...
  async function searchfetcher() {
    ...
    const result = await axios.get(
      `${API}/user/searchuser?${input.includes("@") ? "email" : "skill"}=${input}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 truncate max-w-[175px]">
            Email: {user.email}
          </p>
        </div>
      </div>
